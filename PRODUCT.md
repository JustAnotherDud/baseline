...
Target **WCAG 2.1 AA**, mobile-first. Body text ≥4.5:1, large/bold text ≥3:1 against its actual background (dark theme). Cinzentos medidos 2026-06-15 — **todos passam AA**: `--text3 #9a9a9a` em `--bg` 6.8:1 / em `--surface2` 5.65:1 (pior caso); `--text2 #bbb` 9.98:1; hues de macro a 22px ≥7.5:1; `accent-ink` em botão accent 11.4:1. Interactive touch targets ≥44×44px since the app is used one-handed on a phone. Respect `prefers-reduced-motion` for transitions and Chart.js animations. Color is never the sole carrier of meaning (macro hues are paired with mono labels P/C/F and numeric values). Maintain a visible focus state for any keyboard/assistive use even though primary input is touch.
