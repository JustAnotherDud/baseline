...
- Refeições (7): tap no nome → meal breakdown (donut) ou log se vazia; `+ LOG` → log. Chevron recolhe/expande a refeição (estado em localStorage, esconde as entradas); entradas com tara mostram `⚖ tem tara`.
...
- Recolher/expandir refeições (chevron, estado em localStorage) + flag de tara nas entradas; seletores de refeição gerados de `MEALS`
